...
class _JobMatchesScreenState extends State<JobMatchesScreen> {
  ...
  Future<void> _toggleFavorite(String userId, int index) async {
    try {
      final res = await ApiService().toggleFavorite(userId);
      final isFav = res['isFavorite'] as bool;
      if (mounted) {
        setState(() {
          if (_results != null) {
            final m = _results!.matches[index];
            _results!.matches[index] = JobMatch(
              id: m.id,
              jobId: m.jobId,
              userId: m.userId,
              score: m.score,
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: isFav ? AppColors.primary : Colors.grey.shade700,
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        if (_error != null && (_results == null || _results!.matches.isEmpty))
                          Padding(
                            padding: const EdgeInsets.only(top: 12),
                            child: Container(
