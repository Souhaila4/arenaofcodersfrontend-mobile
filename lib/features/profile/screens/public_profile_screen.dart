...
class _PublicProfileScreenState extends State<PublicProfileScreen> {
  ...
  Future<void> _loadUser() async {
    try {
      ...
      final wins = await _api.getUserWins(widget.userId);
      if (mounted) {
        setState(() {
          _user = user;
          _wins = wins.data;
  ...
  Widget _buildBadgesSection(bool isDark) {
    if (_wins.isEmpty) return const SizedBox();
  ...
  Widget _buildMedalSummary() {
    ...
    for (var win in _wins) {
      final rank = (win as dynamic).winnerRank as int? ?? 1;
      if (rank == 1) gold++;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          _buildSummaryMedal('🥇', gold, 'OR', Colors.amber),
          Container(width: 1, height: 30, color: AppColors.primary.withAlpha(30)),
          _buildSummaryMedal('🥈', silver, 'ARGENT', Colors.grey.shade400),
          Container(width: 1, height: 30, color: AppColors.primary.withAlpha(30)),
