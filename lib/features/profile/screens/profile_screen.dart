...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadWins(String userId) async {
    try {
      final wins = await _api.getUserWins(userId);
      if (mounted) {
        setState(() {
          _wins = wins.data;
        });
      ...
      // Ignore errors for wins loading
    }
  }
        ...
        children: [
          ...
          CustomScrollView(
            slivers: [
                        ...
                        IconButton(
                          onPressed: () {
                            if (Navigator.of(context).canPop()) {
                              ...
                              // If we are in the main shell tabs, go back to Home (index 0)
              ...
              // Winner Badges
              if (_user?.role == 'USER' && _wins.isNotEmpty)
                SliverToBoxAdapter(
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(20, 28, 20, 0),
                    child: Column(
  ...
  Widget _buildMedalSummary() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xFF0D6CF2).withAlpha(15),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: const Color(0xFF0D6CF2).withAlpha(30)),
      ),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: [
          _buildSummaryMedal('🥇', gold, 'GOLD', Colors.amber),
          Container(width: 1, height: 30, color: const Color(0xFF0D6CF2).withAlpha(30)),
          _buildSummaryMedal('🥈', silver, 'SILVER', Colors.grey.shade400),
          Container(width: 1, height: 30, color: const Color(0xFF0D6CF2).withAlpha(30)),
          _buildSummaryMedal('🥉', bronze, 'BRONZE', Colors.brown.shade400),
        ],
      ),
    );
  }

  Widget _buildSummaryMedal(String emoji, int count, String label, Color color) {
    return Column(
      children: [
        Row(
          children: [
            ...
            Text(
              'x$count',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: count > 0 ? color : Colors.grey),
            ),
          ],
        ),
        Text(
          label,
          style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, color: color.withAlpha(150), letterSpacing: 1),
        ),
      ],
    );
  }
