...
class _LeaderboardScreenState extends State<LeaderboardScreen> {
  ...
  void initState() {
    super.initState();
    _leaderboardFuture = Future.value([]); // placeholder
    _loadUserAndData();
  }
  ...
  Future<void> _loadUserAndData() async {
    ...
    if (_currentUser?.role == 'USER' && _currentUser?.mainSpecialty != null) {
      for (var entry in _filterMap.entries) {
        if (entry.value == _currentUser!.mainSpecialty) {
  ...
  Future<List<AuthUser>> _fetchLeaderboard() async {
    try {
      ...
      // Only show normal users in the leaderboard
      parsed = parsed.where((u) => u.role == 'USER').toList();
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    colors: [const Color(0xFF00C2FF).withAlpha(25), Colors.transparent],
                  ),
                ),
              ),
            ),
          ...
          child: Center(
            child: Text(
              ...
              style: TextStyle(
                ...
                color: Colors.white.withAlpha(102), // Modern translucent white
                letterSpacing: -2,
              ),
            ),
          ),
