...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await _api.getNotifications(limit: 100, unreadOnly: true);
      if (mounted) setState(() {
        _list = res.data.where((n) => !n.read).toList();
  ...
  Future<void> _markAsRead(AppNotification n) async {
    ...
    setState(() {
      _list.removeWhere((item) => item.id == n.id);
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return _NotificationTile(
                                ...
                                onTap: () async {
                                  await _markAsRead(n);
                                  // Si c'est une invitation d'équipe, ouvrir l'écran des invitations
                                  if (n.type == 'EQUIPE_INVITATION' && mounted) {
                                    Navigator.of(context).push(
                                      MaterialPageRoute(
                                        builder: (_) => const TeamInvitationsScreen(),
                                      ),
                                    );
                                  } else if (n.competitionId != null && mounted) {
                                    Navigator.of(context).push(
                                      MaterialPageRoute(
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  height: 44,
                  decoration: BoxDecoration(
                    color: notification.type == 'EQUIPE_INVITATION'
                        ? Colors.orange.withAlpha(25)
                        : AppColors.primary.withAlpha(25),
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: Icon(
                    notification.type == 'EQUIPE_INVITATION'
                        ? Icons.group_add
