...
class _HackathonDetailScreenState extends State<HackathonDetailScreen> {
  ...
  void _startPollingScore() {
    _refreshTimer?.cancel();
    // Poll every 5 seconds if submitted but no score yet
    _refreshTimer = Timer.periodic(const Duration(seconds: 5), (_) async {
      if (!mounted) {
        _refreshTimer?.cancel();
        return;
      }
      if (_participantStatus == 'SUBMITTED' && _score == null) {
        await _refreshScoreOnly();
      } else if (_participantStatus == 'SUBMITTED' && _score != null) {
        // We got it! stop polling
        _refreshTimer?.cancel();
      }
    });
  }

  Future<void> _refreshScoreOnly() async {
    try {
      ...
      if (details != null && mounted) {
        setState(() {
          ...
          _score = details['score'] != null ? (details['score'] as num).toDouble() : null;
          _antiCheatScore = details['antiCheatScore'] as int?;
                      ...
                      children: [
                        ...
                        ConstrainedBox(
                          ...
                          child: searchResults.isEmpty
                              ...
                              : ListView.builder(
                                  shrinkWrap: true,
                                  physics: const ClampingScrollPhysics(), // Important inside SingleChildScrollView
                                  ...
                                  children: [
                                    ...
                                    if (_competition!.creator != null) ...[
                                      ...
                                      InkWell(
                                        ...
                                        child: Row(
                                          children: [
                                            Icon(Icons.business, size: 14, color: Colors.blue.shade400),
                                            const SizedBox(width: 6),
                                            Text(
                                              'Organisé par : ${_competition!.creator!.firstName} ${_competition!.creator!.lastName}',
                                              style: TextStyle(
                                                fontSize: 12,
                                            ...
                                            Icon(Icons.arrow_forward_ios, size: 8, color: Colors.blue.shade400),
                                          ],
                                        ),
                                      ),
                                    ],
  ...
  Widget _buildFinalSubmissionSection(bool isDark) {
    if (!_joined || _competition == null) return const SizedBox.shrink();
    ...
    // Window opens 20 minutes before end
    final bool isSubmissionWindowOpen = timeRemaining.inMinutes <= 20 && !timeRemaining.isNegative;
    final bool isTooLate = timeRemaining.isNegative;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: isSubmitted ? Colors.green.withAlpha(80) : AppColors.primary.withAlpha(50),
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Icon(
                isSubmitted ? Icons.check_circle : Icons.rocket_launch,
          ...
          if (isSubmitted) ...[
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      const Text('Score Final :', style: TextStyle(fontWeight: FontWeight.bold)),
                      const SizedBox(width: 8),
                      Text(
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Votre projet a été soumis et analysé par notre IA.',
                    style: TextStyle(fontSize: 13, color: Colors.grey.shade400),
                  ),
                ],
              ),
            ),
            if (_scoringReport != null) _buildScoringDetails(isDark),
          ] else if (isTooLate) ...[
            const Text('Le hackathon est terminé. Les soumissions sont fermées.', style: TextStyle(color: Colors.redAccent)),
          ] else if (!isSubmissionWindowOpen) ...[
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: _CountdownText(
                      targetDate: endDate.subtract(const Duration(minutes: 20)),
                      prefix: 'La soumission finale ouvre dans : ',
            ...
            const SizedBox(height: 8),
            Text(
              'La soumission finale ne sera disponible que durant les 20 dernières minutes du hackathon.',
              style: TextStyle(fontSize: 12, color: Colors.grey.shade500),
            ),
          ] else ...[
            // Window is open!
            if (!isLeader)
              const Text(
                'La fenêtre de soumission est ouverte ! Seul le leader de l\'équipe peut effectuer la soumission finale.',
                style: TextStyle(color: Colors.orangeAccent, fontWeight: FontWeight.w600),
              )
            else ...[
              if (_myEquipe != null && _myEquipe!['status'] == 'READY') ...[
                const Text(
                  'C\'est le moment ! Soumettez le lien GitHub final de votre projet pour évaluation.',
                  style: TextStyle(fontSize: 14, color: Colors.white70),
                ),
                const SizedBox(height: 16),
                _FinalSubmitForm(
                  competitionId: widget.competitionId,
                  onSuccess: () => _load(showSpinner: false),
                ),
              ] else ...[
                const SizedBox(height: 12),
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Action requise : Votre équipe doit comporter au moins 4 membres et être marquée comme "Prête" dans l\'onglet Équipe pour débloquer la soumission finale.',
  ...
  Widget _buildScoringDetails(bool isDark) {
    if (_scoringReport == null) return const SizedBox.shrink();

    final codeJudge = _scoringReport!['codeJudge'] as Map<String, dynamic>?;
    final productJudge = _scoringReport!['productJudge'] as Map<String, dynamic>?;
    final report = _scoringReport!['report'] as Map<String, dynamic>?;
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            const Icon(Icons.psychology, size: 20, color: Colors.purpleAccent),
            const SizedBox(width: 8),
            Text(
              'Rapport d\'analyse IA (Anti-Triche: ${_antiCheatScore ?? 0}%)',
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.purpleAccent),
            ),
          ],
        ),
        const SizedBox(height: 12),
        ...
        // Summary Reasoning
        Text(
          _scoringReport!['reasoning'] ?? 'Analyse en cours...',
          style: TextStyle(
            fontSize: 13,
            height: 1.5,
        ...
        // Highlights & Warnings
        if (report != null) ...[
          if (report['highlights'] != null && (report['highlights'] as List).isNotEmpty) ...[
            _buildListSection(
              title: 'Points Forts',
              items: List<String>.from(report['highlights']),
          ...
          if (report['warnings'] != null && (report['warnings'] as List).isNotEmpty) ...[
            _buildListSection(
              ...
              items: List<String>.from(report['warnings']),
        ...
        ...items.map((item) => Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(child: Text(item, style: const TextStyle(fontSize: 12, color: Colors.white70))),
            ],
          ),
        )).toList(),
      ],
...
class _FinalSubmitForm extends StatefulWidget {
  final String competitionId;
  final VoidCallback onSuccess;

  const _FinalSubmitForm({required this.competitionId, required this.onSuccess});

  @override
  State<_FinalSubmitForm> createState() => _FinalSubmitFormState();
}

class _FinalSubmitFormState extends State<_FinalSubmitForm> {
  final _api = ApiService();
  ...
  Future<void> _submit() async {
    ...
    if (url.isEmpty || !url.contains('github.com')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Veuillez entrer une URL GitHub valide.'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      await _api.submitGithubLink(widget.competitionId, url);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('✅ Projet soumis avec succès ! Analyse en cours...'), backgroundColor: Colors.green),
        );
        widget.onSuccess();
      }
    } on ApiError catch (e) {
    ...
    return Column(
      children: [
        TextField(
          ...
          style: const TextStyle(color: Colors.white),
          decoration: InputDecoration(
            hintText: 'https://github.com/votre-projet',
            hintStyle: TextStyle(color: Colors.grey.shade600),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: _submitting
                ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                : const Text('Soumettre le projet', style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ),
      ],
    );
