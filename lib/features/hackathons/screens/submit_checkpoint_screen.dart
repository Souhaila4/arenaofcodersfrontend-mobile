...
class _SubmitCheckpointScreenState extends State<SubmitCheckpointScreen> {
  ...
  /**
   * PRÉ-VALIDATION ANTI-TRICHE (Étape 1)
   * 
   * Cette fonction s'assure que le participant a bien :
   * 1. Fourni un lien (GitHub).
   * 2. Uploadé une photo de son environnement.
   * 3. Enregistré un message vocal explicatif.
   * 
   * Elle envoie ensuite l'image et l'audio aux modèles d'IA Hugging Face via notre backend.
   * Si l'IA détecte une fraude (ex: voix ne correspond pas au code, environnement avec 2 personnes),
   * le processus s'arrête ici. Sinon, on débloque la soumission finale (_finalizeSubmission).
   */
  ...
  /**
   * SOUMISSION FINALE DU CHECKPOINT (Étape 2 - Avancement GitHub)
   * 
   * Une fois l'Anti-Cheat (Image + Vocal) passé avec succès, cette fonction envoie 
   * l'URL GitHub au backend NestJS.
   * 
   * Le backend va :
   * 1. Vérifier si c'est le CP1 (sauvegarde de l'URL de base) ou un CP suivant (vérifie que l'URL commence par la base).
   * 2. Cloner le repo en mémoire et compter les fichiers (Avancement Temporel Objectif).
   * 3. Si l'avancement est insuffisant -> Consommation d'une vie (Joker) ou Disqualification (REJECTED).
   * 4. Renvoyer le statut d'évaluation (status, warningMessage, rejectionReason).
   */
  Future<void> _finalizeSubmission() async {
    setState(() {
      _loading = true;
      _loadingStep = '📤 Envoi au serveur + évaluation IA en cours...\n(peut prendre 30-60s)';
      _error = null;
    });
    ...
    try {
      ...
      // --- Parse advancedValidation from the backend response ---
      final advancedValidation = response['advancedValidation'] as Map<String, dynamic>?;
      final backendStatus = advancedValidation?['status'] as String?;
      final warningMessage = advancedValidation?['warningMessage'] as String?;
      final rejectionReason = advancedValidation?['rejectionReason'] as String?;

      setState(() { _loading = false; });

      // --- CASE 1: REJECTED (repo mismatch, no advancement, etc.) ---
      if (backendStatus == 'REJECTED') {
        final reason = rejectionReason ?? 'Checkpoint rejeté par le système.';
        await showDialog(
          context: context,
          barrierDismissible: false,
          builder: (ctx) => AlertDialog(
        ...
        // Stay on screen so user can fix and retry
        return;
      }

      // --- CASE 2: APPROVED with WARNING (grace period / joker used) ---
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'CHECKPOINT VALIDÉ AVEC AVERTISSEMENT',
                    style: TextStyle(
                      color: Colors.amber,
                      fontSize: 14,
                      fontWeight: FontWeight.w900,
      ...
      // --- CASE 3: Normal APPROVED (no warning, no rejection) ---
      widget.onSubmitted?.call();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ Checkpoint soumis et validé avec succès !'),
