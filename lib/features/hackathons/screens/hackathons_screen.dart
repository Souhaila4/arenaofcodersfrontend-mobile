  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final results = await Future.wait([
        _api.getCompetitionsForMe(limit: 20),
        _api.getCompetitions(limit: 20),
        _api.getNotifications(limit: 1),
      ]);
      if (mounted) {
        setState(() {
          _forMe = (results[0] as CompetitionsResponse).data;
          _all = (results[1] as CompetitionsResponse).data;
          _unreadNotifications = (results[2] as NotificationsResponse).unreadCount;
          _loading = false;
        });
      }
    } on ApiError catch (e) {
