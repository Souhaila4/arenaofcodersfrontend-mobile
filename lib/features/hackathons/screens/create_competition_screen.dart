...
class _CreateCompetitionScreenState extends State<CreateCompetitionScreen> {
  ...
  final _topNController = TextEditingController(text: '5'); // Added topN
  ...
  // --- Hackathon ideas (from AI) ---
  List<HackathonIdea>? _ideas;
  ...
  Future<void> _fetchIdeas() async {
    ...
    try {
      final list = await _api.getHackathonIdeas();
      if (mounted) setState(() {
        _ideas = list;
        _loadingIdeas = false;
        _ideasError = list.isEmpty ? 'No ideas returned.' : null;
      });
    } on ApiError catch (e) {
      ...
      if (mounted) setState(() {
        _ideasError = e.displayMessage;
        _loadingIdeas = false;
      });
    } catch (_) {
      if (mounted) setState(() {
        _ideasError = 'Failed to load ideas';
        _loadingIdeas = false;
      });
    }
  }

  void _applyIdea(HackathonIdea idea) {
    ...
    if (idea.feasibility.isNotEmpty) {
   ...
   * 
   * Valide les champs (durée minimale de 3h, titre, dates).
   * Envoie ensuite la requête à l'API NestJS pour créer le Hackathon.
   * Le backend se chargera de générer automatiquement les Checkpoints.
   */
  Future<void> _submit() async {
    ...
    if (_endDate!.difference(_startDate!).inHours < 3) {
      setState(() => _error = 'La durée du hackathon doit être d\'au moins 3 heures');
      return;
    }
    ...
    try {
      ...
      final topNVal = int.tryParse(_topNController.text.trim()) ?? 5;
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  FilledButton.icon(
                                    onPressed: _loadingIdeas ? null : _fetchIdeas,
                                    icon: _loadingIdeas
                                        ? const SizedBox(
                                            width: 18,
                                            height: 18,
                                    ...
                                    label: Text(_loadingIdeas ? 'Loading…' : 'Get ideas'),
                              ...
                              if (_ideas != null && _ideas!.isNotEmpty) ...[
                                const SizedBox(height: 16),
                                Text(
                                  'Tap a card to use it as title & description:',
                                  style: TextStyle(fontSize: 12, color: Colors.grey.shade400),
                                ),
                                const SizedBox(height: 10),
                                ...(_ideas!.map((idea) => _ideaCard(context, idea, isDark))),
                              ],
                            ],
                          ),
                        ),
                        ...
                        const SizedBox(height: 16),
                        _label('Nombre de gagnants (Top N)*'),
                        const SizedBox(height: 6),
                        TextFormField(
                          controller: _topNController,
                          keyboardType: TextInputType.number,
                          style: TextStyle(color: isDark ? Colors.white : AppColors.textLightPrimary, fontSize: 15),
                          decoration: _inputDecoration(context, 'Ex: 5 (Nombre de participants dans le Top)'),
                          validator: (v) {
                            final val = int.tryParse(v ?? '');
                            if (val == null || val < 1) return 'Doit être >= 1';
                            return null;
                          },
                        ),
                        // --- Anti Cheat Section ---
  ...
  Widget _ideaCard(BuildContext context, HackathonIdea idea, bool isDark) {
    return Padding(
              ...
              children: [
                ...
                if (idea.description.isNotEmpty) ...[
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 13,
                      color: isDark ? Colors.grey.shade300 : AppColors.textLightSecondary,
                    ),
                  ),
                ],
                if (idea.targetMarket.isNotEmpty) ...[
