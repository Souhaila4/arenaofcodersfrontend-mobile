...
class _SignUpScreenState extends State<SignUpScreen>
  ...
  Future<void> _pickAvatar() async {
    ...
    if (result != null) {
      if (result.files.single.size > 5 * 1024 * 1024) {
        setState(() => _errorMessage = 'Image too large (max 5MB)');
        return;
      }
      setState(() {
        _selectedAvatar = result.files.single;
        _errorMessage = null;
      });
    }
  }
  ...
  Future<void> _handleSignUp() async {
    ...
    if (_selectedFile == null) {
      setState(() => _errorMessage = 'Please select a resume (.docx)');
      return;
    }

    if (_selectedAvatar == null) {
      setState(() => _errorMessage = 'Please upload a profile photo (Avatar) for facial verification');
      return;
    }
    ...
    try {
      ...
      // Log the user in immediately after sign up since email verification is skipped
      await _api.signIn(
        email: _emailController.text.trim(),
        password: _passwordController.text,
      );
      ...
      if (!mounted) return;
      // Navigate directly to the app home
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [
                              BoxShadow(color: AppColors.primary.withAlpha(30), blurRadius: 20),
                            ],
                            image: _selectedAvatar != null && _selectedAvatar!.bytes != null
                                ? DecorationImage(
                                    image: MemoryImage(_selectedAvatar!.bytes!),
                                    fit: BoxFit.cover,
                                  )
                                : null,
                          ),
                          child: _selectedAvatar == null
                              ? const Column(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(height: 4),
                                    Text('Avatar', style: TextStyle(color: AppColors.primary, fontSize: 10, fontWeight: FontWeight.bold)),
                                  ],
                                )
                              : null,
                        ),
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  _selectedFile != null ? _selectedFile!.name : 'Upload Resume (.docx)',
                                  style: TextStyle(
                                    color: _selectedFile != null ? Colors.white : Colors.grey.shade600,
                                    fontSize: 14,
                                  ),
                                ),
                              ),
