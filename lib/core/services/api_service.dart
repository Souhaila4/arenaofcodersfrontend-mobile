...
class ApiService {
  ...
  static String get baseUrl {
    if (kIsWeb) {
      return 'http://192.168.100.87:3000';
  ...
  /// Validate Workspace Image using HuggingFace AI Model
  Future<Map<String, dynamic>> validateWorkspaceImage(
    ...
    final request = http.MultipartRequest('POST', uri);
    request.headers['Authorization'] = 'Bearer ${Env.huggingFaceToken}';
    ...
    if (response.statusCode == 200) {
      ...
      throw ApiError(statusCode: response.statusCode, message: 'AI Validation failed (Status ${response.statusCode})');
    }
  }

  /// Validate Audio check using HuggingFace Vocal AI Model
  Future<Map<String, dynamic>> validateVocalAudio(List<int> audioBytes, String audioFilename) async {
    final uri = Uri.parse('https://negzaoui-modelevocal.hf.space/scan-audio');
    final request = http.MultipartRequest('POST', uri);
    request.headers['Authorization'] = 'Bearer ${Env.huggingFaceToken}';
    ...
    if (response.statusCode == 200) {
      try {
        ...
        throw ApiError(statusCode: response.statusCode, message: 'Invalid AI JSON Response');
      }
    } else {
      throw ApiError(statusCode: response.statusCode, message: 'AI Vocal Validation failed (Status ${response.statusCode})');
    }
  }
