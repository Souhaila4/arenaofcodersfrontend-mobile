...
class ApiService {
  ...
  /// Get competitions won by a specific user
  Future<CompetitionsResponse> getUserWins(String userId) async {
    final token = await _storage.getToken();
    if (token == null) throw ApiError(statusCode: 401, message: 'Not authenticated');

    final response = await http.get(
      Uri.parse('$baseUrl/competitions/user/$userId/wins'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<Map<String, dynamic>> selectWinner(String competitionId, String participantId, int rank) async {
    final token = await _storage.getToken();
    if (token == null) throw ApiError(statusCode: 401, message: 'Not authenticated');
  ...
  /// Ajouter ou supprimer un talent des favoris (COMPANY uniquement)
  Future<Map<String, dynamic>> toggleFavorite(String userId) async {
    final token = await _storage.getToken();
    if (token == null) throw ApiError(statusCode: 401, message: 'Not authenticated');

    final response = await http.post(
      Uri.parse('$baseUrl/jobs/favorites/$userId/toggle'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  /// Récupérer tous les talents favoris (COMPANY uniquement)
  Future<List<Map<String, dynamic>>> getFavoriteUsers() async {
    final token = await _storage.getToken();
    if (token == null) throw ApiError(statusCode: 401, message: 'Not authenticated');

    final response = await http.get(
      Uri.parse('$baseUrl/jobs/favorites'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
